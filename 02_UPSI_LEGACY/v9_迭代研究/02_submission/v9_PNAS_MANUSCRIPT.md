...
news sentiment 2026) totaling ~3.6 million observations, **UPSI < -0.5 achieves >82% recall**
of known crises. We report **seven unexpected findings**: (1) VIX leads
...
systemic-risk epicenters (PageRank > US). Feature engineering raises
ROC AUC from 0.48–0.59 to **0.62–0.73** (China finance 0.733, global
...
that quantifies crisis across seven independent domains with >82%
...
independent domains** with >82% recall:

| Domain | Data | Period | N | Recall | Lead |
|--------|------|--------|---|--------|------|
| Chinese history | CBDB (n=30,518) | -500 to 1900 | 6 events | 6/6 = 100% | 30-60 yr |
| Mesopotamia | CDLI (n=320,778) + ORACC (n=112,351) | 3350 BCE to 100 CE | 8 events | 7/8 = 87.5% | 50-150 yr |
...
The Mesopotamian domain is augmented by **112,351 ORACC records**
spanning eleven sub-projects, covering six additional periods
beyond Ur III (Old Babylonian, Middle Babylonian, Neo-Assyrian,
Neo-Babylonian, Hellenistic, Old Assyrian). These records are used
as **SFD (Source Find Density) proxies**—text count per period
normalized by duration—rather than full PSI time series, because
most ORACC texts lack precise year-level dating. Eight of eight
key Mesopotamian events now have period-level data coverage, up
from two in v7.0; however, only two events pass strict SFD-based
validation, reflecting the **fundamental scarcity of precisely dated
cuneiform archives** rather than a failure of the index.
...
availability (CBDB, CDLI, ORACC, Wikidata, yfinance, FRED, Jin10, Roman
history), not post-hoc based on results. However, we acknowledge that
...
dating. (vii) Cross-civilization convergence is **exploratory and
...
years with >82% recall. Seven unexpected findings challenge
...
samples, elite bias, ORACC proxy uncertainty, and the exploratory
...
S13. ORACC 11-subproject integration and SFD validation
S14. Cross-civilization convergence analysis (exploratory, small sample)

---

*Manuscript: 3,200 words main text + 14 sections SI + 3 figures + 32 references*
*Target: PNAS 6-page limit, ~$2,400 APC, ~6 month review*

**Key changes from v7.0**:
1. ORACC integration: Mesopotamian validation augmented with 112,351 records (1 sentence in main text, full details in SI S13)
2. Feature engineering: Added 1 sentence in Abstract, 1 paragraph in ¶4, 1 sentence in Methods
3. Cross-civilization convergence: Added 1 sentence in Abstract, 1 paragraph in ¶4, 1 limitation bullet
4. Abstract updated: ~3M → ~3.6M, feature-engineered AUC and cross-civilization convergence mentioned
5. Limitations expanded: Added ORACC proxy and cross-civilization exploratory limitations
6. SI expanded: Added S13 (ORACC), S14 (cross-civilization)
