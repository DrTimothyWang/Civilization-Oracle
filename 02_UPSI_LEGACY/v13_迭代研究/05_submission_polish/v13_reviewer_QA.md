...
## Q0.1: "SPI seems post-hoc — how do we know it wasn't designed to fit those 2 failures?"

**Answer**: SPI is **mathematically distinct** from PSI, not a parameter tweak:

| Property | PSI | SPI |
|----------|-----|-----|
| Mathematical operation | Integration (smoothing) | Differentiation (sharpening) |
| Time scale | 50–100 years | 1–10 years |
| Signal type | Level (z-score of count) | Rate of change (velocity, acceleration) |
| Detection mode | Trough (low = crisis) | Spike (high = crisis) |
| Physical analogy | Temperature | dT/dt |

SPI was motivated by **theoretical boundary analysis** (v12.0): we identified that PSI's 50–100 year windows inherently suppress intra-window variance, making burst crises invisible. This is a **signal-processing principle**, not data-fitting. The formula (velocity + acceleration + geographic velocity + volatility spike) is standard in change-point detection and control theory; we did not invent new mathematics to fit two data points.

**Honest admission**: SPI's weights (0.35/0.25/0.25/0.15) are theoretically motivated but not cross-validated on independent data. They should be treated as a **proof-of-concept** requiring future validation on additional burst crises (e.g., Roman Crisis of the Third Century, An Lushan Rebellion).

---

## Q0.2: "The 4-quadrant UPSI_v2 classification seems arbitrary — why these 4 regimes?"

**Answer**: The four quadrants are derived from **state-space dynamics**, not arbitrary categorization. Any 2D system with a level variable (PSI) and a rate variable (SPI) has four natural quadrants:
...
This is isomorphic to classical mechanics (position + velocity phase portraits) and control theory (state-space representation). The quadrant labels are interpretive, but the partition itself is mathematically necessary.

**Honest admission**: The quadrant boundaries (thresholds for "high" vs "low") are heuristic (mean ± 1.5σ). They should be calibrated on a larger crisis database before operational deployment. We present the quadrants as a **conceptual framework**, not a validated classification system.

---

## Q0.3: "Does SPI replace PSI? Should we discard PSI?"

**Answer**: **No. SPI complements PSI; neither replaces the other.**

| Regime | PSI | SPI | Why both needed |
|--------|-----|-----|-----------------|
| Gradual decline (Tang, Ming) | ✅ Captures | ⚠️ Noisy | PSI is primary |
| Sudden collapse (Hammurabi, Umma) | ❌ Misses | ✅ Captures | SPI is primary |
| Stable prosperity | ✅ Captures | ✅ Normal | Both agree |
| Recovery/renaissance | ⚠️ Ambiguous | ⚠️ Ambiguous | Both ambiguous |

In sparse-data regimes (ancient history), PSI dominates because SPI requires temporal resolution. In high-resolution regimes (modern finance, COVID-19), SPI dominates because burst crises are more relevant. The **confidence-weighted UPSI_v2** automatically adjusts: w_SPI increases with exact-year data ratio (w_SPI = 0.8 for modern finance, w_SPI = 0.1 for Old Babylonian).

**Analogy**: A doctor uses both thermometer (PSI = temperature) and heart-rate monitor (SPI = rate of change). Neither replaces the other; together they provide a complete vital-signs picture.
