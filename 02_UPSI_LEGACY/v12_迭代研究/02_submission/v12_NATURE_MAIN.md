...
**Date**: 2026-06-04
**Version**: v12.0 (v11 base + Bayesian hierarchical validation + high-pressure prosperity paradox + Seshat 8th domain plan)
...
#### ¶2.6 Bayesian hierarchical validation (150 words)
...
invasion—another abrupt crisis invisible to coarse-grained proxies. These
cases do not invalidate UPSI; they identify boundary conditions where
proxy composition (genre weights) and temporal resolution (multi-scale
windows) must be refined. We propose a dual-track architecture—Gradual
Pressure Index (GPI) for slow-burn crises and Sudden Pressure Index (SPI)
for abrupt shocks—as a future theoretical direction.

Future work will integrate the Seshat Global History Databank (30+
NGAs, 12,000 years) as an eighth independent domain, testing UPSI
across structured historical codings (SI Section 18).
