...
**Date**: 2026-06-04
**Version**: v10.0 (v9 base + v9a ORACC multi-period validation corrected, 6/8=75.0%)
...
| 2 | Mesopotamia | CDLI (n=320,778) + ORACC (n=112,351) | 3350 BCE to 100 CE | **6/8 (75.0%)** | 50-150 yr |
...
Umma decline (Ur III) and Hammurabi's posthumous fragmentation
...
to fragment after Hammurabi's death. These failures are not
index failures but **proxy-teaching moments**: they reveal that
SFD proxies capture archive productivity, not necessarily
political stability, and that genre-diversity metrics (not just
count) are needed for robust fragmentation signals.
...
**Key changes from v9.0 manuscript**:
1. Mesopotamian validation corrected: v9.0 incorrectly reported 7/8 (87.5%) based on v7 CDLI data; v10.0 reports the actual v9a ORACC multi-period result 6/8 (75.0%)
2. ORACC integration honestly framed: 6/8 pass rate reflects broader period coverage (7 periods), not higher accuracy; 2 failures (Umma, Hammurabi) discussed as proxy-teaching moments
3. Abstract updated: recall threshold ≥82% → ≥75% to reflect honest Mesopotamian reporting
4. Results ¶2.2 expanded: Added explicit discussion of Umma and Hammurabi failures, SFD proxy limitations, and data-sparse periods (Neo-Babylonian n=512, Middle Babylonian n=657)
5. Discussion limitations expanded: Added fifth limitation bullet on ORACC proxy failures and their interpretive value
6. Methods expanded: Added SFD proxy formula (PSI_proxy = 0.6×SFD_z - 0.4×GSI_cv_z) and adaptive window specification
